...
**Reason**: Split into the store-mediated flow — M3U import now populates the store, and the sandbox rebuild is an export from the store.
**Migration**: Run `import --format m3u` to populate the store from `.m3u8` input, then `export --format nml` to rebuild the managed sandbox.
