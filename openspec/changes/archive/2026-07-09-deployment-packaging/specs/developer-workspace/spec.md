...
The repository SHALL provide standardized local commands for formatting, linting, type checking, tests, common workflow operations, and buildable runtime artifact verification.
...
#### Scenario: Developer verifies runtime artifact outputs
- **WHEN** a developer wants to validate the deployable runtime outputs
- **THEN** the repository exposes standardized commands or checks that verify the Nix package and app surfaces in addition to the local Python checks
